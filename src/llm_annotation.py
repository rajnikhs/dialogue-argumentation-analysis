...
# The exact 22 valid schemes matching your prompt
...
# Strict OpenAI JSON schema mirroring your Literal types to prevent label collapse
RESPONSE_SCHEMA = {
    "type": "json_schema",
    "json_schema": {
        "name": "scheme_classification",
        "strict": True,
        "schema": {
            "type": "object",
            "properties": {
            ...
            "required": ["inferred_scheme"],
            "additionalProperties": False,
        },
    },
}

def infer_scheme(client, model_name, text):
    # Your exact prompt text preserved completely
...
"combat operations in Afghanistan finished in 2014. Mehdi Hasan is living in the past. Mehdi Hasan is massively misrepresenting the Afghanistan situation."
...
"politicians knew. that is the worst part about it, to see politicians this week saying, "I'm shocked at this collapse, shocked it didn't work out the way ... "."
...
"businesses are closing all over the place. the hospitality industry is literally on its knees."
...
"we should have learned from what happened in Vietnam. we haven't done well but we haven't done as badly as some other countries."
...
"women get far more abuse than men in social media and black people. Naomi Osaka is a black woman and she gets tremendous abuse, no doubt on social media."
...
"the Afghanistan war is on the same level of a failure as the Iraq war. what we've done in Afghanistan seriously deserves a huge accounting."
...
"teachers have worked so hard over the last few months. the teachers of Sam Unkown's boys have been such a credit."
...
"Kevin Collin's report is very in line with what all the other countries similar to Britain and our committees are calling for. Kevin Collins report was very obvious what it was going to report for quite a long time leading up to it."
...
"there have been a series of political and strategic failures over a very long period of time that led to here. the current situation has long and deep roots."
...
"levelling up is not about investing in our people. we don't know what levelling up is for."
...
"those decisions were flawed in that there was no real understanding of the complexity of what then had to come next. the current situation has long and deep roots."
...
"everybody wanted to talk about mental health. we spent the whole session on mental health."
...
"Jersey is an area in which customarily French fishermen did fish. the French fishermen in Jersey are not people suddenly turning up saying "Can we fish here?"."
...
"Anthony Costello wishes we had not had lockdowns and followed a much more oppressive policy. Anthony Costello is not arguing for another national lockdown."
...
"Nelufar Hedayat went to a place called Skater Stan, and she met young girls wearing their traditional Afghan clothes going up and down these huge skateparks that they had built because the war in Afghanistan gave a generation hope. the war in Afghanistan was fought to some degree for the children."
...
"the teachers of Sam Unkown's boys have worked so hard with the kids. it is so unfair that the funding for school catch-up is not going to be there."
...
"unless we start putting leverage on the Taliban in order to put pressure on them not to allow Afghanistan to descend into a haven for terrorists. we will reap the benefits."
...
"ultimately, the fans are what the game is all about. Robert Jenrick hopes we can use this as an opportunity to make the game more dynamic, more democratic and more in the interests of the fan."
...
"we haven't done well but we haven't done as badly as some other countries. let's move forward, get the population vaccinated, get out of it and move forward with sensible ideas that will allow us to regain our lives."
...
"the Taliban are worried in the next month or six weeks, there will be a NATO attack, some sort of land - there will be troops on the ground. the Taliban are on their best behaviour."
...
"we need to make sure that we embed the successes that we have had. there is still work to do."
...
"people will not be skilled for the jobs that will be needed in the post-COVID economy. in five years' time the education budget cut will be failing business."

Classify the following argument into one of the 22 classes.
...
Scheme:"""
    ...
    return parsed.get("inferred_scheme", "Error")

def run_annotation(model_id, base_url, input_path, output_path):
    print("Running constrained inference across the dataset via vLLM...")
    ...
    for idx, row in df.iterrows():
        ...
        try:
            result = infer_scheme(client, model_id, text)
    ...
    print(f"--- Pipeline Complete! Saved strictly constrained results to {output_path} ---")
